...
/** Created by Hamilton Mutaquiha on 06/10/19. Utility methods for reporting */
public class EmrReportingUtils {
   ...
   * @return the cohort indicator
   */
  public static CohortIndicator cohortIndicator(
      String name,
      Mapped<CohortDefinition> numeratorCohort,
  ...
  /**
   * Adds a row to a dataset based on an indicator and a list of column parameters (generates column
   * name from the column parameters)
   *
  ...
  public static void addRow(
    ...
    for (ColumnParameters column : columns) {
      String name =
          baseName + "-" + (columnNames != null ? columnNames.get(c++) : column.getName());
